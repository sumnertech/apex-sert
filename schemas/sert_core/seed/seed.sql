...
insert into sert_core.rule_criteria (rule_criteria_name, rule_criteria_key, rule_criteria_type_id, rule_criteria_sql, reason) values
  ('Usage of HTP without SYS prefix', 'USAGE_OF_HTP_WITHOUT_SYS_PREFIX', (select rule_criteria_type_id from sert_core.rule_criteria_types where rule_criteria_type_key = 'SQLI'), 'with string as (select :l_source as s from dual) select count(*) from string where regexp_count(string.s,''htp\.'',1,''i'') > regexp_count(string.s,''(^|[^s]sys\.)htp\.'',1,''i'')', 'Be sure to include the SYS prefix when making calls to HTP');
insert into sert_core.rule_criteria (rule_criteria_name, rule_criteria_key, rule_criteria_type_id, rule_criteria_sql, reason) values
